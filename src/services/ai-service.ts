  ...
  return value
    .map((item) => {
      ...
      return {
        label,
        type,
        rules: typeof record.rules === 'string' && record.rules.trim() ? record.rules.trim() : 'AI 生成标签',
...
class AiService {
  ...
  async enrichEvent(input: {
    ...
    return this.requestStructured({
      model: input.model,
      prompt: buildEventEnrichmentPrompt(input),
      temperature: 0.3,
      maxTokens: 900,
      validate: (value) => {
        const record = asRecord(value);
  ...
  async generateFriendComment(input: {
    ...
    return this.requestStructured({
      model: input.model,
      prompt: buildFriendCommentPrompt(input),
      temperature: 0.8,
      maxTokens: 500,
      validate: (value) => {
        const record = asRecord(value);
